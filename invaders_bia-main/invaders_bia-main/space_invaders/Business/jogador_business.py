"""
Módulo JogadorBusiness - Camada de Negócio (Business Logic)

Este módulo contém as REGRAS DE NEGÓCIO relacionadas ao jogador.
Responsabilidades:
- Movimentação do jogador (esquerda, direita, cima, baixo)
- Lógica de disparo de projéteis
- Atualização de tiros do jogador

Separação de Responsabilidades (SRP):
- Camada de DADOS (jogador.py): Armazena atributos e validações básicas
- Camada de NEGÓCIO (este arquivo): Lógica de comportamento e ações

Observação: As validações de limite de tela estão nas properties
da classe Jogador, demonstrando encapsulamento adequado.
"""
