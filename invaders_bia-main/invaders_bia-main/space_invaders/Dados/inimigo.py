"""
Módulo Inimigo - Camada de Dados (Model)

Este módulo contém a classe Inimigo, que representa as entidades
inimigas (alienígenas) no jogo. Cada inimigo possui:
- Posição (x, y)
- Dimensões (largura, altura)
- Tipo (1, 2 ou 3 - define sprite e pontuação)
- Direção de movimento

Princípios de POO aplicados:
1. ENCAPSULAMENTO: Atributos privados protegidos
2. PROPERTIES: Acesso controlado com validação de direção
3. IMUTABILIDADE PARCIAL: Tipo e dimensões são somente leitura após criação
"""
...
class Inimigo:
    ...
    @property
    def sprite(self):
        """Getter para o sprite do inimigo (pode ser None)."""
