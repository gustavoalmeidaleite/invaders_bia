"""
Módulo JogoHeadless - Orquestrador Principal SEM Interface Gráfica

Este módulo contém uma versão do jogo que funciona sem renderização gráfica.
É utilizado pelo webservice Flask para:
- Processar comandos recebidos via rede (Socket.IO ou REST)
- Manter o estado lógico do jogo
- Retornar o estado atual para renderização no cliente (navegador)

Diferenças para jogo.py:
- Não inicializa display do Pygame
- Não carrega sprites/imagens
- Recebe comandos via método processar_comando() ao invés de eventos
- Retorna estado via método obter_estado() para serialização JSON

Padrões Utilizados:
- FACADE: Fornece interface simplificada para o webservice
- STATE MACHINE: Gerencia estados (menu, jogando, game_over)
- COMMAND PATTERN: Comandos recebidos como strings

Para executar via web: python -m space_invaders.web.main
"""
